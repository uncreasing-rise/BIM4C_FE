...
if (!email || !password)
  throw new Error("Bootstrap credentials are not configured");
...
try {
  ...
  check(
    "FE login and HttpOnly session",
    result.response.status,
    200,
    result.response.status === 200 && cookie.startsWith("bim4c_admin_session="),
  );
  ...
  check(
    ...
    result.response.status === 200 &&
      result.body?.data?.roles?.includes("SUPER_ADMIN"),
  );
  result = await request(
    "/api/admin/homepage/slides/00000000-0000-4000-8000-000000000000",
    {
      ...json("PATCH", { title: "Cross-origin" }),
      headers: {
  ...
  result = await request(
    ...
    json("POST", {
      eyebrow: "CODEX E2E",
      title: "Temporary E2E slide",
      image: "/images/about.jpg",
      alt: "Temporary E2E image",
      sortOrder: 9999,
      isActive: true,
    }),
  );
  ...
  check(
    ...
    result.response.status,
    201,
    result.response.status === 201 && temporary.slide,
  );
  result = await request(
    `/api/admin/homepage/slides/${temporary.slide}`,
    json("PATCH", {
      title: "Temporary E2E slide updated",
      isActive: false,
      sortOrder: 9998,
    }),
  );
  check(
    "Slides UPDATE/order/toggle",
    result.response.status,
    200,
    result.response.status === 200 &&
      result.body?.title === "Temporary E2E slide updated" &&
      result.body?.isActive === false,
  );
  result = await request(
    "/api/admin/homepage/slides",
    json("POST", { title: "x" }),
  ...
  result = await request(
    "/api/admin/homepage/slides/00000000-0000-4000-8000-000000000000",
    json("PATCH", { title: "Missing" }),
  );
  check("Slides missing id", result.response.status, 404);
  result = await request(`/api/admin/homepage/slides/${temporary.slide}`, {
    method: "DELETE",
  });
  ...
  result = await request(
    "/api/admin/homepage/partners",
    json("POST", {
      name: "Temporary E2E partner",
  ...
  check(
    ...
    result.response.status,
    201,
    result.response.status === 201 && temporary.partner,
  );
  result = await request(
    `/api/admin/homepage/partners/${temporary.partner}`,
    json("PATCH", {
      name: "Temporary E2E partner updated",
      isActive: false,
      sortOrder: 9998,
    }),
  );
  check(
    "Partners UPDATE/order/toggle",
    result.response.status,
    200,
    result.response.status === 200 &&
      result.body?.name === "Temporary E2E partner updated" &&
      result.body?.isActive === false,
  );
  result = await request(`/api/admin/homepage/partners/${temporary.partner}`, {
    method: "DELETE",
  });
  ...
  form.set(
    "file",
    new File(
      [await readFile("public/images/about.jpg")],
      "codex-e2e-about.jpg",
      { type: "image/jpeg" },
    ),
  );
  form.set("alt", "Temporary E2E media");
  result = await request("/api/admin/media/upload", {
    method: "POST",
    body: form,
  });
  ...
  if (temporary.media) {
    ...
    const optimized = await fetch(
      `${frontend}/_next/image?url=${encodeURIComponent(temporary.media.url)}&w=640&q=75`,
    );
    check("Next Image render", optimized.status, 200);
    result = await request("/api/admin/media?search=codex-e2e-about.jpg");
    check(
      "Media metadata reload",
      result.response.status,
      200,
      result.response.status === 200 &&
        result.body?.data?.some((item) => item.id === temporary.media.id),
    );
    result = await request(`/api/admin/media/${temporary.media.id}`, {
      method: "DELETE",
    });
    ...
    const deletedObject = await fetch(
      `${temporary.media.url}?cleanup=${Date.now()}`,
      { cache: "no-store" },
    );
    check(
      "Supabase object cleanup",
      deletedObject.status,
      "400/404",
      [400, 404].includes(deletedObject.status),
    );
  ...
  check(
    ...
    201,
    result.response.status === 201 &&
      /^[a-f0-9-]{36}\.jpg$/i.test(dangerous?.storageKey ?? ""),
  );
  if (dangerous?.id)
    await request(`/api/admin/media/${dangerous.id}`, { method: "DELETE" });
  const duplicateA = await upload(jpeg, "duplicate.jpg", "image/jpeg");
  const duplicateB = await upload(jpeg, "duplicate.jpg", "image/jpeg");
  check(
    "Duplicate filenames get unique keys",
    duplicateB.response.status,
    201,
    duplicateA.response.status === 201 &&
      duplicateB.response.status === 201 &&
      duplicateA.body?.data?.storageKey !== duplicateB.body?.data?.storageKey,
  );
  for (const item of [duplicateA.body?.data, duplicateB.body?.data])
    if (item?.id)
      await request(`/api/admin/media/${item.id}`, { method: "DELETE" });

  const invalidCookie = cookie;
  cookie =
    "bim4c_admin_session=invalid-session-token-that-is-long-enough-123456789";
  result = await request("/api/admin/homepage/slides");
  ...
  if (temporary.slide)
    await request(`/api/admin/homepage/slides/${temporary.slide}`, {
      method: "DELETE",
    });
  if (temporary.partner)
    await request(`/api/admin/homepage/partners/${temporary.partner}`, {
      method: "DELETE",
    });
  if (temporary.media?.id)
    await request(`/api/admin/media/${temporary.media.id}`, {
      method: "DELETE",
    });
}
