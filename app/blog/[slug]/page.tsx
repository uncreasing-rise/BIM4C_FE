...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!entry) return {};
  const canonical = env.appUrl ? `${env.appUrl}${ROUTES.blogDetail(slug)}` : ROUTES.blogDetail(slug);
  return { title: `${entry.title} | BIM4C`, description: entry.description, alternates: { canonical }, openGraph: { type: "article", title: entry.title, description: entry.description, url: canonical, images: [{ url: entry.image, alt: entry.title }] }, twitter: { card: "summary_large_image", title: entry.title, description: entry.description, images: [entry.image] } };
}
