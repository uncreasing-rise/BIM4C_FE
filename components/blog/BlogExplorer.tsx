...
export function BlogExplorer({ posts }: { posts: ContentEntry[] }) {
  ...
  return <section className="w-full bg-[#f5fafa] py-12 lg:py-16"><div className="mx-auto w-[calc(100%_-_32px)] max-w-[1440px] md:w-[calc(100%_-_48px)] lg:w-[calc(100%_-_80px)]">
    ...
    {pages > 1 && <nav className="mt-[42px] flex justify-center gap-2" aria-label="Phân trang tin tức"><button className={`${pageButton} disabled:opacity-35`} type="button" disabled={page === 1} onClick={() => setPage((value) => value - 1)}>←</button>{Array.from({ length: pages }, (_, index) => index + 1).map((number) => <button type="button" className={`${pageButton} ${page === number ? "border-[#09a7a5] bg-[#09a7a5] text-white" : "bg-white text-[#09a7a5]"}`} aria-current={page === number ? "page" : undefined} onClick={() => setPage(number)} key={number}>{String(number).padStart(2, "0")}</button>)}<button className={`${pageButton} disabled:opacity-35`} type="button" disabled={page === pages} onClick={() => setPage((value) => value + 1)}>→</button></nav>}
  </div></section>;
}
