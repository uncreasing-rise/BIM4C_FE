...
export function BlogExplorer({ posts }: { posts: ContentEntry[] }) {
  ...
  const filtered = useMemo(() => posts.filter((item) => (category === "Tất cả" || item.eyebrow === category) && (!query || `${item.title} ${item.description}`.toLocaleLowerCase("vi").includes(query.toLocaleLowerCase("vi")))), [posts, category, query]);
  const pages = Math.max(1, Math.ceil(filtered.length / pageSize));
  const visible = filtered.slice((page - 1) * pageSize, page * pageSize), featured = visible.slice(0, 3), standard = visible.slice(3);
