      ...
      {blocks.map((block) => {
        switch (block.type) {
          ...
          case "image":
            return (
              <figure className="overflow-hidden rounded-2xl" key={block.id}>
                ...
                />
                {block.image.caption && (
                  <figcaption className="bg-muted px-4 py-3 text-center text-xs text-muted-foreground">
                    {block.image.caption}
                  </figcaption>
                )}
              </figure>
            ...
            return (
              ...
              >
                {block.images.map((image, index) => (
                  <figure
                    className={`relative overflow-hidden rounded-2xl bg-muted ${index === 0 && block.images.length % 2 ? "sm:col-span-2" : ""}`}
                    key={`${image.url}-${index}`}
                  >
                    <Image
                    ...
                    />
                    {image.caption && (
                      <figcaption className="absolute inset-x-0 bottom-0 bg-black/60 p-3 text-xs text-white">
                        {image.caption}
                      </figcaption>
                    )}
                  </figure>
          ...
          case "quote":
            return (
              <blockquote
                className="border-l-4 border-primary bg-muted/50 px-6 py-7 text-2xl font-medium leading-relaxed"
                key={block.id}
              >
                <p>“{block.quote}”</p>
                {block.author && (
