...
export function CourseExplorer({ courses }: { courses: ContentEntry[] }) {
  ...
  return (
    <section
      className={`public-explorer course-explorer ${styles.page}`}
      aria-label="Các chương trình đào tạo"
    >
      <div className={styles.section}>
        <header className={styles.heading}>
          <p className={styles.headingKicker}>Chương trình đào tạo</p>
          <h2 className={styles.headingTitle}>Năng lực có thể áp dụng ngay</h2>
          <p className={styles.headingDescription}>
            Chọn chương trình phù hợp với công việc và mục tiêu phát triển của
            bạn.
          </p>
        </header>
        <nav className="public-category-nav" aria-label="Danh mục đào tạo">
          {categories.map((item) => (
            <button
              type="button"
        ...
        <div className="public-filter-bar mb-5">
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((course) => (
            <article
              className={`group relative flex ${styles.card}`}
              id={course.slug}
              key={course.slug}
            >
              <Link
              ...
              <div className={styles.cardBody}>
                <p className={styles.cardKicker}>{course.eyebrow}</p>
                ...
                <span className={styles.cardLink}>Xem chương trình →</span>
              </div>
            </article>
          ))}
        </div>
        {filtered.length === 0 && (
          <p className="border-t border-[#e9e6e3] py-10 text-center text-[14px] text-[#777]">
