...
export function Projects({ projects }: { projects: Project[] }) {
  ...
  const navigate = (step: -1 | 1) => {
    ...
    setMotion(step);
    window.setTimeout(() => move(step), 400);
    window.setTimeout(() => setMotion(0), 1200);
  };
      ...
      {visible.map((project, index) => <article style={{ transitionDelay: `${index * 35}ms` }} className={`group relative h-full min-w-0 transform-gpu overflow-hidden bg-[#667775] transition-[transform,opacity] duration-700 ease-[cubic-bezier(.22,1,.36,1)] motion-reduce:transition-none ${motion === -1 ? "translate-x-1.5 opacity-90" : motion === 1 ? "-translate-x-1.5 opacity-90" : "translate-x-0 opacity-100"} ${desktopCardVisibility[index]} ${index > 0 && index < 4 ? "min-h-[420px] lg:min-h-[560px]" : ""}`} key={`${project.slug}-${index}`}>
        <Link className="absolute inset-0 block focus-visible:outline-2 focus-visible:outline-offset-[-3px] focus-visible:outline-white" href={ROUTES.projectDetail(project.slug)} aria-label={`Xem dự án ${project.title}`}>
          ...
          {index > 0 && index < 4 && <div className="absolute inset-x-0 bottom-0 p-5 text-white lg:p-7">
            <h3 className="m-0 text-2xl font-bold uppercase leading-snug [text-shadow:0_1px_2px_rgb(0_0_0/.35)]">{project.title}</h3>
            <div className="max-h-0 translate-y-3 overflow-hidden opacity-0 transition-[max-height,opacity,transform,margin] duration-[1000ms] ease-[cubic-bezier(.22,1,.36,1)] group-hover:mt-4 group-hover:max-h-64 group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:mt-4 group-focus-within:max-h-64 group-focus-within:translate-y-0 group-focus-within:opacity-100">
              <p className="mb-3 text-micro font-normal uppercase tracking-[.12em] text-white/75">{project.category}</p>
              ...
              <span className="mt-5 inline-flex min-h-11 items-center gap-3 border border-white/45 bg-white/10 px-5 text-[15px] font-normal uppercase tracking-[.06em] text-white backdrop-blur-sm transition-[background-color,border-color,color] duration-500 hover:border-[#09a7a5] hover:bg-[#09a7a5] hover:text-white">XEM DỰ ÁN <b className="font-normal transition-transform duration-500 group-hover:translate-x-1">→</b></span>
            </div>
          </div>}
        </Link>
      </article>)}
