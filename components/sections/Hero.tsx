...
export function Hero({ slides }: { slides: HeroSlide[] }) {
  ...
  return <section className="relative isolate flex h-[calc(100svh-68px)] min-h-[600px] w-full items-center overflow-hidden bg-[#063f46] text-white lg:h-[calc(100svh-84px)] lg:min-h-[640px]" onTouchStart={(event) => { touch.current = event.touches[0].clientX; }} onTouchEnd={(event) => { const distance = event.changedTouches[0].clientX - touch.current; if (Math.abs(distance) > 45) move(distance < 0 ? 1 : -1); }}>
    {slides.map((item, index) => <Image className={`hero-slide-transition object-cover object-center saturate-[.92] contrast-[1.05] transition-[opacity,transform] motion-reduce:transition-none ${index === active ? "scale-100 opacity-100" : "scale-[1.04] opacity-0"}`} key={item.id ?? `${item.image}-${item.sortOrder}-${index}`} src={item.image} alt={index === active ? item.alt : ""} aria-hidden={index !== active} fill preload={index === 0} sizes="100vw" />)}
    <div className="absolute inset-y-0 left-0 z-[1] w-[92%] bg-[linear-gradient(90deg,rgba(6,63,70,.96)_0%,rgba(6,63,70,.78)_58%,transparent_100%)] sm:w-[80%] lg:w-[66%]" />
    ...
    <div className="absolute inset-x-0 bottom-0 z-[1] h-28 bg-gradient-to-t from-[#063f46]/80 to-transparent" />
    ...
    <div className="relative z-[2] mx-auto grid w-[calc(100%_-_40px)] max-w-[1400px] items-center gap-8 pb-14 pt-8 sm:w-[calc(100%_-_64px)] lg:w-[calc(100%_-_96px)] lg:grid-cols-[minmax(0,1fr)_280px] lg:pb-16">
      <div key={active} className="max-w-[780px] motion-safe:animate-[content-enter_.8s_cubic-bezier(.22,1,.36,1)_both]">
        ...
        <div className="mt-8 flex flex-wrap items-center gap-x-8 gap-y-4 lg:mt-10">
          <Link className="button-primary group" href={ROUTES.services}>Khám phá năng lực <span className="text-lg transition-transform group-hover:translate-x-1">→</span></Link>
