...
export function HomepageManager() {
  ...
  return (
    <section className="overflow-hidden rounded-md border border-[#dbe7e5] bg-white shadow-sm">
      <header className="flex flex-col gap-2 border-b border-[#dbe7e5] p-4 sm:flex-row sm:justify-between [&>div]:flex [&>div]:gap-1.5 [&_button]:min-h-[38px] [&_button]:border [&_button]:border-[#dbe7e5] [&_button]:px-3">
        <div>
          <button
            className={
              tab === "slides"
                ? "border-[#09a7a5] bg-[#eaf8f7] text-[#09a7a5]"
                : ""
            }
            onClick={() => {
              setTab("slides");
      ...
      {loading ? (
        <div className="p-16 text-center text-[#667775]">
        ...
        <>
          {selected && isSlide(selected) && (
            <div className="relative m-4 h-[260px] overflow-hidden text-white md:h-[390px] [&>img]:object-cover [&>div]:absolute [&>div]:inset-0 [&>div]:bg-gradient-to-r [&>div]:from-[#063f46]/90 [&>span]:absolute [&>span]:left-6 [&>span]:top-5 [&>span]:text-micro [&>section]:absolute [&>section]:bottom-8 [&>section]:left-6 [&>section]:max-w-xl [&_h2]:mt-2 [&_h2]:text-3xl [&>button]:absolute [&>button]:bottom-4 [&>button]:right-4 [&>button]:bg-white [&>button]:p-2.5 [&>button]:text-[#09a7a5]">
              <Image
                src={selected.image}
                alt={selected.alt}
                fill
          ...
          <div className="grid gap-2 p-4 [&_article]:flex [&_article]:flex-wrap [&_article]:items-center [&_article]:gap-3 [&_article]:border [&_article]:border-[#dbe7e5] [&_article]:p-2 [&_article>div]:flex [&_article>div]:min-w-0 [&_article>div]:flex-1 [&_article>div]:flex-col [&_nav]:flex [&_nav]:gap-1 [&_nav_button]:h-8 [&_nav_button]:min-w-8 [&_nav_button]:border [&_nav_button]:border-[#dbe7e5]">
            {items.map(
              (item, index) =>
                isSlide(item) && (
                  <article
                    className={
                      item.id === selected?.id ? "border-[#09a7a5]" : ""
                    }
                    key={item.id}
        ...
        <div className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2 lg:grid-cols-3 [&_article]:border [&_article]:border-[#dbe7e5] [&_article]:p-3.5 [&_article>div]:relative [&_article>div]:my-2 [&_article>div]:h-24 [&_article>div]:bg-[#f5fafa] [&_img]:object-contain [&_nav]:mt-3 [&_nav]:flex [&_nav]:gap-1 [&_nav_button]:h-8 [&_nav_button]:min-w-8 [&_nav_button]:border [&_nav_button]:border-[#dbe7e5]">
          {items.map(
            (item, index) =>
              !isSlide(item) && (
                <article
      ...
      {editing && (
        <>
          <button
            ...
            aria-label="Đóng"
          />
          <aside className="fixed inset-y-0 right-0 z-[60] flex w-full max-w-[600px] flex-col bg-white shadow-2xl [&>header]:flex [&>header]:justify-between [&>header]:border-b [&>header]:border-[#dbe7e5] [&>header]:p-5 [&>footer]:mt-auto [&>footer]:flex [&>footer]:justify-end [&>footer]:gap-2 [&>footer]:border-t [&>footer]:border-[#dbe7e5] [&>footer]:p-4">
            <header>
              <div>
            ...
            <div className="grid flex-1 gap-4 overflow-y-auto p-5 [&_label]:grid [&_label]:gap-1.5 [&_label]:text-xs [&_input]:border [&_input]:border-[#dbe7e5] [&_input]:p-2.5 [&_select]:border [&_select]:border-[#dbe7e5] [&_select]:p-2.5 [&_textarea]:border [&_textarea]:border-[#dbe7e5] [&_textarea]:p-2.5">
              <div className="relative h-[230px] bg-[#f5fafa] [&_img]:object-contain">
                <Image
