...
export function ProjectExplorer({ projects }: { projects: Project[] }) {
  ...
  return <section className="w-full bg-[#f5fafa] py-14 lg:py-20" id="project-list"><div className="mx-auto w-[calc(100%_-_32px)] max-w-[1440px] md:w-[calc(100%_-_48px)] lg:w-[calc(100%_-_80px)]">
    ...
    <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">{visible.map((project) => <article className="group relative aspect-[16/10] overflow-hidden bg-[#063f46] shadow-sm" key={project.slug}>
      ...
      <div className="absolute inset-x-0 bottom-0 p-5 text-white lg:p-6">
        <h2 className="text-[20px] font-bold uppercase leading-tight">{project.title}</h2>
        <div className="max-h-0 translate-y-3 overflow-hidden opacity-0 transition-[max-height,opacity,transform,margin] ease-[cubic-bezier(.22,1,.36,1)] group-hover:mt-4 group-hover:max-h-56 group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:mt-4 group-focus-within:max-h-56 group-focus-within:translate-y-0 group-focus-within:opacity-100">
          <p className="mb-3 text-[15px] font-normal uppercase tracking-wider text-white">{project.category}</p>
          ...
          <span className="mt-4 inline-flex min-h-11 items-center gap-3 border border-white/45 bg-white/10 px-5 text-[15px] font-normal uppercase text-white backdrop-blur-sm transition-colors hover:border-[#09a7a5] hover:bg-[#09a7a5]">Xem dự án <b className="font-normal">→</b></span>
        </div>
      </div>
      </Link>
