...
export function Header() {
  ...
  return <header className="sticky top-0 z-50 h-[68px] w-full bg-white shadow-[0_1px_0_rgb(4_95_94_/_15%)] lg:h-[85px]">
    ...
    <div className="flex h-[68px] items-center px-5 lg:h-[50px] lg:grid lg:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] lg:gap-6 lg:px-[clamp(24px,3vw,48px)] xl:gap-12 xl:px-[clamp(28px,4vw,68px)]">
      <nav className="hidden h-full min-w-0 items-center justify-self-end gap-3 text-[13px] font-bold tracking-[.02em] lg:flex xl:gap-[clamp(14px,1.5vw,26px)] xl:text-[15px] xl:tracking-[.03em]" aria-label="Điều hướng chính"><NavMenu label="GIỚI THIỆU" href={ROUTES.about} items={HEADER_MENUS.about} pathname={pathname} /><NavMenu label="DỊCH VỤ" href={ROUTES.services} items={HEADER_MENUS.services} pathname={pathname} /><NavMenu label="DỰ ÁN" href={ROUTES.projects} items={HEADER_MENUS.projects} pathname={pathname} /></nav>
      ...
      <nav className="hidden h-full min-w-0 items-center justify-self-start gap-3 text-[13px] font-bold tracking-[.02em] lg:flex xl:gap-[clamp(14px,1.5vw,26px)] xl:text-[15px] xl:tracking-[.03em]" aria-label="Điều hướng bổ sung"><Link className={linkClass(ROUTES.courses)} aria-current={isCurrent(pathname, ROUTES.courses) ? "page" : undefined} href={ROUTES.courses}>KHÓA HỌC</Link><Link className={linkClass(ROUTES.blog)} aria-current={isCurrent(pathname, ROUTES.blog) ? "page" : undefined} href={ROUTES.blog}>TIN TỨC</Link><Link className={linkClass(ROUTES.contact)} href={ROUTES.contact}>LIÊN HỆ</Link></nav>
      <MobileNavigation />
    </div>
  </header>;
}
