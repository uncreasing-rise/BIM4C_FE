...
function NavMenu({ label, href, items, pathname }: { label: string; href: string; items: ReadonlyArray<{ label: string; href: string }>; pathname: string }) {
  ...
  return <div className="group relative flex h-full items-center">
    ...
    <div className="invisible absolute left-0 top-[42px] w-[280px] translate-y-3 border-t-[3px] border-[#09a7a5] bg-white p-3 opacity-0 shadow-[0_12px_30px_rgb(3_68_67_/_18%)] transition-[opacity,transform,visibility] duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:visible group-focus-within:translate-y-0 group-focus-within:opacity-100">
...
export function Header() {
  ...
  return <header className="sticky top-0 z-50 h-[68px] w-full bg-white shadow-[0_1px_0_rgb(4_95_94_/_15%)] lg:h-[84px]">
    <div className="hidden h-[42px] bg-[#063f46] text-[11px] font-semibold tracking-[.06em] text-white lg:block"><div className="mx-auto flex h-full w-[calc(100%_-_48px)] max-w-[1440px] items-center justify-between"><Link className="tracking-[.04em] transition-colors hover:text-[#09a7a5]" href={ROUTES.contact}>HỢP TÁC CÙNG CHÚNG TÔI</Link><nav className="flex h-full items-center" aria-label="Liên kết tiện ích"><Link className="border-l border-white/15 px-4 transition-colors hover:text-[#09a7a5]" href={ROUTES.courses}>TUYỂN DỤNG</Link><Link className="border-l border-white/15 px-4 transition-colors hover:text-[#09a7a5]" href={ROUTES.contact}>LIÊN HỆ</Link><a className="border-l border-white/15 px-4 transition-colors hover:text-[#09a7a5]" href="#">UNITED RUN</a><div className="group relative flex h-full cursor-pointer items-center border-l border-white/15 px-4"><span className="flex items-center gap-1">TIẾNG VIỆT <span className="text-[10px]">⌄</span></span><div className="invisible absolute right-0 top-full w-[120px] translate-y-1.5 bg-[#063f46] p-1.5 opacity-0 transition group-hover:visible group-hover:translate-y-0 group-hover:opacity-100">{["Tiếng Việt", "English"].map((language) => <a className="block px-2.5 py-1.5 hover:bg-white/10" href="#" key={language}>{language}</a>)}</div></div></nav></div></div>
    <div className="flex h-[68px] items-center px-5 lg:h-[42px] lg:grid lg:grid-cols-[auto_auto_auto] lg:justify-center lg:gap-12 lg:px-[clamp(28px,4vw,68px)]">
      ...
      <Link className="mr-auto justify-self-center text-3xl font-bold leading-none tracking-[-.03em] drop-shadow-sm text-[#063f46] lg:mr-0" href={ROUTES.home}>BIM<span className="text-[#09a7a5]">4C</span></Link>
