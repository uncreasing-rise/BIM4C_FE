  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (error instanceof z.ZodError)
        setMessage(error.issues[0]?.message ?? "Thông tin chưa hợp lệ.");
      else
        setMessage(
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      ...
      <div className="flex">
        ...
        />
        <button
          className="h-[46px] w-12 bg-[#09a7a5] text-xl font-bold text-[#063f46] disabled:cursor-wait disabled:opacity-60"
          type="submit"
      ...
      <label className="mt-3 flex items-start gap-2 text-micro leading-normal text-white/70">
        <input className="mt-0.5" name="consent" type="checkbox" required /> Tôi
        đồng ý nhận thông tin từ BIM4C.
      </label>
      {message && (
        <p
