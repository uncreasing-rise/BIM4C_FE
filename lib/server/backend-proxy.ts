...
export async function backendProxy(request: NextRequest, path: string) {
  ...
  const headers = new Headers({ Accept: request.headers.get("accept") ?? "application/json", Origin: process.env.FRONTEND_URL ?? request.nextUrl.origin });
  ...
  try {
    response = await fetch(target, { method: request.method, headers, body: ["GET", "HEAD"].includes(request.method) ? undefined : await request.arrayBuffer(), cache: "no-store", redirect: "manual" });
  } catch (error) {
