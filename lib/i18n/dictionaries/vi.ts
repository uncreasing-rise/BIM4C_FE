import type { Dictionary } from "../types";

/** Repairs Vietnamese strings that were accidentally decoded as ANSI. */
function repairMojibake(value: unknown): unknown {
    if (typeof value === "string") {
        if (!/[ÃÂÄÅÆÐÑĂ]|á(?:»|º)|â(?:€|€“|-)|�/u.test(value)) {
            return value;
        }

        const bytes = Uint8Array.from(value, (character) => character.charCodeAt(0));
        return new TextDecoder("utf-8", { fatal: false }).decode(bytes);
    }

    if (Array.isArray(value)) {
        return value.map(repairMojibake);
    }

    if (value && typeof value === "object") {
        return Object.fromEntries(
            Object.entries(value).map(([key, entry]) => [key, repairMojibake(entry)]),
        );
    }

    return value;
}

const viDictionaryRaw: Dictionary = {
    common: {
        talkToExpert: "Trao đổi với chuyên gia",
        contact: "Liên hệ",
        ourWork: "Dự án của chúng tôi",
        exploreExpertise: "Khám phá năng lực chuyên môn",
        discussProject: "Trao đổi về dự án",
        viewDetails: "Xem chi tiết",
        readMore: "Đọc thêm",
        learnMore: "Tìm hiểu thêm",
        all: "Tất cả",
        search: "Tìm kiếm",
        searchPlaceholder: "Tìm kiếm...",
        filterByCategory: "Danh mục",
        filterByStatus: "Trạng thái",
        filterByLocation: "Địa điểm",
        filterByYear: "Năm",
        back: "Quay lại",
        skipToContent: "Chuyển đến nội dung chính",
        loading: "Đang tải...",
        noResults: "Không tìm thấy kết quả.",
        publishedAt: "Ngày đăng",
        duration: "Thời lượng",
        level: "Trình độ",
        instructor: "Giảng viên",
        curriculum: "Chương trình học",
        learningOutcomes: "Kết quả học tập",
        scopeOfServices: "Phạm vi dịch vụ",
        deliverables: "Sản phẩm bàn giao",
        projectOverview: "Tổng quan dự án",
        projectDetails: "Chi tiết dự án",
        investor: "Chủ đầu tư",
        location: "Địa điểm",
        scale: "Quy mô",
        year: "Năm",
        status: "Trạng thái",
        relatedProjects: "Dự án liên quan",
        relatedServices: "Giải pháp liên quan",
        relatedCourses: "Khóa học liên quan",
        relatedPosts: "Bài viết liên quan",
        share: "Chia sẻ",
        clearFilters: "Xóa bộ lọc",
        downloadProfile: "Tải hồ sơ năng lực (PDF)",
        backToTop: "Lên đầu trang",
        copied: "Đã sao chép!",
        copyAddress: "Sao chép địa chỉ",
        copyTaxCode: "Sao chép mã số thuế",
    },
    navigation: {
        home: "Trang chủ",
        about: "Giới thiệu",
        services: "Giải pháp",
        projects: "Dự án",
        courses: "Học viện",
        blog: "Kiến thức",
        bimViewer: "Khám phá BIM 3D",
        contact: "Liên hệ",
        legal: "Pháp lý",
        terms: "Điều khoản",
        privacy: "Bảo mật",
        exploreBim4c: "Khám phá BIM4C",
        tagline: "Chuyển đổi số xây dựng",
        requestConsultation: "Yêu cầu tư vấn",
    },
    hero: {
        badge: "HỆ SINH THÁI DỮ LIỆU XÂY DỰNG SỐ · OPENBIM & CDE",
        titleMain: "Hạ tầng dữ liệu BIM.",
        titleHighlight: "Chính xác. Khả thi. Bàn giao số.",
        description:
            "Tư vấn và triển khai BIM, tích hợp công nghệ số vào thiết kế, thi công và quản lý vận hành tài sản.",
        ctaPrimary: "Yêu cầu tư vấn giải pháp",
        ctaSecondary: "Khám phá dự án tiêu biểu",
        featureBimStrategy: "Quy trình ISO 19650 / BEP",
        featureCoordination: "Kiểm soát xung đột đa bộ môn trên CDE",
        featureDigitalHandover: "COBie & Digital Twin as-built",
        cardTag: "Kinh nghiệm dự án",
        cardTagline: "MÔ HÌNH HÓA. PHỐI HỢP. BÀN GIAO.",
        bimViewport: {
            coordinateSystem: "Hệ tọa độ VN2000 / XYZ",
            viewCube: "Góc nhìn phối cảnh",
            clashAlert: "Phối hợp mô hình đa bộ môn",
            ifcInspector: "Trình kiểm tra đối tượng IFC4",
            liveModelStatus: "Minh họa mô hình BIM",
            layers: {
                arch: "Kiến trúc (ARC)",
                struct: "Kết cấu (STR)",
                mep: "Cơ điện (MEP)",
                timeline: "Tiến độ 4D",
            },
        },
    },
    expertiseStrip: {
        bimConsulting: {
            title: "Tư vấn BIM",
            desc: "Tiêu chuẩn, kế hoạch triển khai và quản lý vòng đời dự án",
        },
        training: {
            title: "Đào tạo",
            desc: "Kỹ năng thực tế cho đội ngũ dự án và kỹ sư",
        },
        design: {
            title: "Thiết kế",
            desc: "Phối hợp đa bộ môn và kiểm tra khả năng thi công",
        },
        supervision: {
            title: "Giám sát",
            desc: "Kiểm tra số, an toàn công trường và kiểm soát chất lượng",
        },
    },
    deliveryProcess: {
        eyebrow: "Cách chúng tôi làm việc",
        title: "Quy trình rõ ràng từ khởi động đến bàn giao",
        description:
            "Mỗi dự án đều tuân theo một phương pháp luận có cấu trúc để đảm bảo tính nhất quán của dữ liệu, giảm thiểu sai sót và tối ưu hiệu quả đầu tư.",
        step1: {
            title: "1. Đánh giá & Định hướng",
            desc: "Xác định mục tiêu, đánh giá mức độ sẵn sàng BIM và xây dựng kế hoạch triển khai phù hợp (BEP/EIR).",
        },
        step2: {
            title: "2. Mô hình hóa & Tích hợp",
            desc: "Xây dựng mô hình đa bộ môn tuân thủ nghiêm ngặt tiêu chuẩn LOD và quy tắc đặt tên.",
        },
        step3: {
            title: "3. Phối hợp & Rà soát",
            desc: "Phát hiện xung đột sớm, tổ chức các buổi làm việc liên bộ môn và theo dõi xử lý vấn đề đến khi hoàn tất.",
        },
        step4: {
            title: "4. Bàn giao & Vận hành",
            desc: "Bàn giao mô hình as-built đã xác thực và dữ liệu tài sản có cấu trúc, sẵn sàng cho quản lý vận hành.",
        },
    },
    partners: {
        eyebrow: "Được các doanh nghiệp hàng đầu tin tưởng",
        title: "Khách hàng & Đối tác",
        description:
            "Chúng tôi hợp tác với các chủ đầu tư, tổng thầu và đơn vị tư vấn thiết kế hàng đầu trên khắp Việt Nam.",
    },
    consultation: {
        eyebrow: "Liên hệ với chúng tôi",
        title: "Sẵn sàng bắt đầu hành trình BIM của bạn?",
        description:
            "Trao đổi trực tiếp với đội ngũ kỹ thuật của chúng tôi để đánh giá yêu cầu dự án và nhận đề xuất triển khai phù hợp.",
        nameLabel: "Họ và tên",
        emailLabel: "Email công việc",
        phoneLabel: "Số điện thoại",
        serviceLabel: "Giải pháp quan tâm",
        messageLabel: "Mô tả dự án",
        submitButton: "Gửi yêu cầu",
        submitting: "Đang gửi...",
        successMessage: "Cảm ơn bạn! Chuyên gia BIM của chúng tôi sẽ sớm liên hệ.",
    },
    footer: {
        description:
            "Kết nối con người, quy trình và dữ liệu dự án để thiết kế, thi công và quản lý tài sản tốt hơn.",
        exploreTitle: "Khám phá",
        newsletterTitle: "Nhận thông tin BIM",
        newsletterDesc:
            "Tin tức dự án và kiến thức chuyển đổi số xây dựng, gửi trực tiếp đến hộp thư của bạn.",
        emailPlaceholder: "Nhập email công việc của bạn...",
        subscribeButton: "Đăng ký",
        consentText: "Tôi đồng ý nhận thông tin liên lạc từ BIM4C.",
        privacyLink: "Chính sách bảo mật",
        copyright:
            "© 2026 CÔNG TY CỔ PHẦN CÔNG NGHỆ VÀ XÂY DỰNG BIM4C (BIM4C JSC). Bảo lưu mọi quyền.",
        termsLink: "Điều khoản dịch vụ",
        privacyPolicyLink: "Chính sách bảo mật",
        enterpriseInfo: {
            companyName: "Công ty Cổ phần Công nghệ và Xây dựng BIM4C",
            internationalName: "BIM4C TECHNOLOGY & CONSTRUCTION JOINT STOCK COMPANY",
            shortName: "BIM4C JSC",
            taxId: "",
            businessCode: "",
            taxStatus: "",
            taxAddress: "",
            headquarters: "Văn phòng: 20 Bắc Sơn, Đà Nẵng",
            branchOffice: "",
            legalRepresentative: "TRẦN NGỌC HIẾU",
            certificationsTitle: "Tiêu chuẩn & Công nghệ áp dụng",
            isoCert: "",
            buildingSmartCert: "",
            autodeskCert: "Bộ giải pháp Autodesk (Revit, Navisworks & ACC)",
        },
    },
    aboutPage: {
        eyebrow: "Về BIM4C",
        heroTitle: "Chuyên môn xây dựng. Kết nối bằng BIM.",
        heroDesc:
            "Chúng tôi kết nối con người, quy trình và thông tin dự án để hỗ trợ thiết kế, thi công và vận hành tốt hơn.",
        letter: {
            title: "Lời ngỏ từ BIM4C",
            paragraphs: [
                "Từ giai đoạn hoạch định đầu tư, thiết kế cho đến thi công và quản lý tài sản, chúng tôi đồng hành cùng khách hàng trong quá trình chuyển đổi số bằng những giải pháp thiết thực, hiệu quả. Với cam kết đổi mới và không ngừng hoàn thiện, BIM4C giúp các tổ chức xây dựng hạ tầng thông minh hơn, bền vững hơn và sẵn sàng cho tương lai thông qua việc tích hợp công nghệ vào mọi giai đoạn của dự án.",
                "Công ty Cổ phần Công nghệ và Xây dựng BIM4C là đơn vị tư vấn và thi công tập trung vào chuyển đổi số trong ngành xây dựng. Chúng tôi kết hợp tư vấn chuyên môn với việc triển khai thực tế các công nghệ như Mô hình thông tin công trình (BIM) nhằm nâng cao hiệu quả, tối ưu quy trình làm việc và tăng cường phối hợp.",
                "Tại BIM4C, mô hình hóa 3D là nền tảng cốt lõi để tích hợp thông tin về vật liệu, tiến độ, chi phí và hiệu suất vận hành. Trí tuệ nhân tạo, học máy và IoT hỗ trợ phân tích mô hình, rà soát xung đột, dự báo rủi ro và luồng thông tin công trường.",
                "BIM4C kết nối BIM với digital twin để tạo ra bản sao kỹ thuật số của tài sản vật lý, hỗ trợ mô phỏng, dự báo, vận hành thông minh, bảo trì dự đoán và tối ưu tài sản trong suốt vòng đời dự án.",
                "Công việc của chúng tôi bắt đầu từ những quyết định mà đội ngũ dự án cần đưa ra: thiết kế gì, phối hợp ra sao, xây dựng và kiểm tra thế nào, bàn giao thông tin gì và vận hành tài sản trong tương lai ra sao. Chúng tôi xác định yêu cầu thông tin, trách nhiệm và điểm kiểm tra trước khi lựa chọn công cụ, để công nghệ luôn gắn với kết quả dự án.",
                "BIM4C đồng hành cùng chủ đầu tư, đơn vị tư vấn, nhà thầu và đơn vị vận hành để biến các bản vẽ, mô hình và hồ sơ rời rạc thành nguồn thông tin đáng tin cậy. Kết quả là một nền tảng số thiết thực mà đội ngũ có thể tiếp nhận, đo lường và tiếp tục duy trì sau khi bàn giao.",
            ],
        },
        visionMission: {
            vision: { title: "Tầm nhìn", text: "BIM4C hướng tới trở thành đối tác tham chiếu đáng tin cậy trên toàn cầu về chuyển đổi số thực tiễn trong ngành xây dựng — đưa thông tin chuẩn xác, hợp tác cởi mở và đổi mới có thể đo lường đến gần hơn với mọi tổ chức. Chúng tôi hướng tới một môi trường xây dựng trong đó dữ liệu tiếp tục tạo giá trị từ hoạch định đầu tư, thiết kế, thi công đến vận hành và cải tạo công trình." },
            mission: { title: "Sứ mệnh", text: "BIM4C thúc đẩy chuyển đổi số ngành xây dựng bằng cách kết nối con người, quy trình và thông tin quanh những quyết định rõ ràng của dự án. Chúng tôi xây dựng tiêu chuẩn BIM khả thi, điều phối dữ liệu đa bộ môn, củng cố quản trị CDE và hỗ trợ bàn giao số để khách hàng tăng khả năng dự báo, bảo vệ ngân sách, giảm lãng phí và vận hành tài sản hiệu quả hơn. Mỗi dự án đều hướng tới việc để lại năng lực mà đội ngũ có thể tự duy trì và tiếp tục cải tiến." },
        },
        workMethod: {
            eyebrow: "Cách chúng tôi làm việc", title: "Đổi mới, hợp tác và lấy khách hàng làm trung tâm", intro: "Chúng tôi điều chỉnh chiến lược theo nhu cầu từng dự án, khuyến khích các góc nhìn đa chiều, phản ứng linh hoạt và theo đuổi các giải pháp sáng tạo mang lại giá trị thiết thực.",
            items: [
                { title: "Lấy khách hàng làm trung tâm", text: "Thấu hiểu nhu cầu của từng khách hàng và điều chỉnh chiến lược phù hợp với yêu cầu cụ thể của dự án." },
                { title: "Văn hóa hợp tác", text: "Duy trì giao tiếp cởi mở và phối hợp chặt chẽ giữa các nhóm hướng tới mục tiêu chung của dự án." },
                { title: "Quản lý linh hoạt", text: "Phản ứng nhanh trước thay đổi và các tình huống phức tạp trong khi vẫn đảm bảo chất lượng." },
                { title: "Đổi mới sáng tạo", text: "Theo sát sự phát triển công nghệ để đưa ra các chiến lược tiên phong cho khách hàng." },
            ],
        },
        operation: {
            eyebrow: "Cách chúng tôi vận hành", title: "Từ nhu cầu đến giải pháp triển khai được", items: [
                { title: "Đánh giá nhu cầu toàn diện", text: "Xác định yêu cầu và thách thức để giải pháp phù hợp với dự án và sử dụng nguồn lực hiệu quả." },
                { title: "Hoạch định chiến lược", text: "Kết nối mục tiêu khách hàng, bối cảnh ngành và mức độ phức tạp của dự án với lộ trình công nghệ phù hợp." },
                { title: "Công nghệ tiên tiến", text: "Tích hợp BIM và các công nghệ phù hợp để tạo ra mô hình thông tin 3D minh bạch, dễ quản lý." },
                { title: "Phát triển linh hoạt", text: "Làm việc theo từng giai đoạn, liên tục cải tiến và phản ứng nhanh khi nhu cầu khách hàng thay đổi." },
            ],
        },
        whyChoose: {
            eyebrow: "TẠI SAO CHỌN BIM4C",
            title: "Năng lực tạo giá trị lâu dài cho dự án",
            intro: "Ba nền tảng giúp BIM4C trở thành đối tác tin cậy trong hành trình chuyển đổi số xây dựng.",
            items: [
                { title: "Chuyên môn và giải pháp tùy chỉnh", text: "Đội ngũ chuyên gia BIM của chúng tôi tùy chỉnh giải pháp cho từng dự án, kết hợp kiến thức chuyên sâu về BIM với việc không ngừng cập nhật công nghệ xây dựng." },
                { title: "Hiệu suất cao và công nghệ tiên tiến", text: "Chúng tôi ứng dụng BIM để tinh gọn quy trình thi công, nâng cao hiệu quả bàn giao và không ngừng cải tiến cách thức triển khai dự án." },
                { title: "Đối tác chiến lược và hỗ trợ chuyên môn", text: "Chúng tôi đồng hành lâu dài cùng khách hàng, cung cấp hỗ trợ chuyên môn và quy trình BIM tối ưu chi phí, hướng tới thành công bền vững." },
            ],
        },
        trackRecord: {
            eyebrow: "NĂNG LỰC BIM4C",
            title: "Dịch vụ xuyên suốt vòng đời công trình",
            metrics: [
                {
                    value: "3D–7D",
                    label: "Dịch vụ BIM",
                    subtext: "Mô hình, tiến độ, chi phí, năng lượng và vận hành",
                },
                {
                    value: "LiDAR",
                    label: "Quét Laser",
                    subtext: "Khảo sát hiện trạng và Scan-to-BIM",
                },
                {
                    value: "Thiết kế",
                    label: "Kiến trúc & Kỹ thuật",
                    subtext: "Nội thất, cảnh quan và hạ tầng",
                },
                {
                    value: "Đào tạo",
                    label: "Chuyển giao công nghệ",
                    subtext: "Chủ đầu tư, quản lý dự án, doanh nghiệp và sinh viên",
                },
            ],
        },
        whoWeAreEyebrow: "Chúng tôi là ai",
        whoWeAreTitle: "Đối tác thực tiễn cho chuyển đổi số xây dựng.",
        whoWeAreP1:
            "BIM4C kết hợp kiến thức xây dựng với Mô hình thông tin công trình, phối hợp và quản lý thông tin. Chúng tôi giúp chủ đầu tư, đơn vị tư vấn và nhà thầu ứng dụng BIM hiệu quả vào công việc hàng ngày.",
        whoWeAreP2:
            "Công việc của chúng tôi trải rộng từ tư vấn, thiết kế đa bộ môn, phối hợp mô hình, đào tạo đến bàn giao số. Mỗi dự án đều bắt đầu từ mục tiêu và những người chịu trách nhiệm triển khai.",
        check1: "Phạm vi phù hợp với giai đoạn dự án của bạn",
        check2: "Quy trình mà đội ngũ của bạn có thể sử dụng và duy trì",
        check3: "Thông tin được chuẩn bị sẵn sàng cho mục đích tiếp theo",
        guidesEyebrow: "Điều dẫn lối chúng tôi",
        guidesTitle: "Những nguyên tắc thể hiện qua công việc.",
        guidesDesc:
            "Công nghệ chỉ thực sự phục vụ dự án khi trách nhiệm, thông tin và quyết định được xác định rõ ràng.",
        values: {
            integrity: {
                title: "Chính trực",
                desc: "Xây dựng quan hệ hợp tác dựa trên sự tin tưởng, minh bạch và trung thực.",
            },
            innovation: {
                title: "Làm việc thông minh",
                desc: "Sử dụng BIM để tổ chức công việc, giảm sai sót và nâng cao hiệu quả.",
            },
            collaboration: {
                title: "Tận tâm",
                desc: "Hỗ trợ khách hàng bằng sự tận tâm và đồng hành cùng đối tác vượt qua thách thức.",
            },
        },
        teamEyebrow: "Ban lãnh đạo & Chuyên gia cấp cao",
        teamTitle: "Đội ngũ kỹ thuật với kinh nghiệm thực chiến.",
        teamDesc:
            "Đội ngũ quản lý và thiết kế của BIM4C, theo hồ sơ năng lực công ty năm 2026.",
        teamMembers: [
            {
                name: "Trần Ngọc Hiếu",
                role: "Nhà sáng lập / CEO",
                cert: "",
                image: "/images/team/avatar-default.jpg",
                spec: "Tư vấn, triển khai và đào tạo BIM.",
            },
            {
                name: "Nguyễn Vũ Long",
                role: "BIM Manager / Đồng sáng lập",
                cert: "",
                image: "/images/team/avatar-default.jpg",
                spec: "Hơn 8 năm kinh nghiệm trong ngành xây dựng.",
            },
            {
                name: "Nguyễn Tiến Cường",
                role: "BIM Coordinator / Đồng sáng lập",
                cert: "",
                image: "/images/team/avatar-default.jpg",
                spec: "Hơn 9 năm kinh nghiệm trong ngành xây dựng.",
            },
            {
                name: "Nguyễn Văn Chiêu",
                role: "BIM Developer / IT Manager",
                cert: "",
                image: "/images/team/avatar-default.jpg",
                spec: "Hơn 6 năm kinh nghiệm trong ngành xây dựng; đã phát triển hơn 100 add-in BIM.",
            },
            {
                name: "Nguyễn Hồng Chinh",
                role: "Trưởng nhóm Thiết kế Kết cấu",
                cert: "",
                image: "/images/team/avatar-default.jpg",
                spec: "Hơn 14 năm kinh nghiệm trong ngành xây dựng.",
            },
            {
                name: "Nguyễn Ngọc Phước",
                role: "Trưởng nhóm Thiết kế MEP & PCCC",
                cert: "",
                image: "/images/team/avatar-default.jpg",
                spec: "Hơn 13 năm kinh nghiệm thiết kế hệ thống công trình.",
            },
            {
                name: "Phan Thị Kiều Diệu",
                role: "Trưởng nhóm Dự toán & Quản lý Chi phí",
                cert: "",
                image: "/images/team/avatar-female.jpg",
                spec: "Hơn 14 năm kinh nghiệm quản lý chi phí xây dựng.",
            },
        ],
        ctaEyebrow: "Hợp tác cùng BIM4C",
        ctaTitle: "Một lộ trình rõ ràng hơn cho dự án tiếp theo của bạn.",
    },
    projectsPage: {
        eyebrow: "Danh mục dự án BIM4C",
        title: "Dự án",
        description:
            "Các dự án tiêu biểu thể hiện cách BIM4C biến sự phối hợp thành sự tin cậy trong bàn giao.",
        catalogueTitle: "Tìm dự án tương tự của bạn",
        catalogueDesc:
            "Duyệt dự án theo loại hình, địa điểm, năm và trạng thái bàn giao.",
        searchLabel: "Tên dự án",
        searchPlaceholder: "Tìm kiếm dự án...",
        locationFilter: "Địa điểm",
        yearFilter: "Năm",
        statusFilter: "Trạng thái bàn giao",
        exploreProject: "Khám phá dự án",
        showingText: (shown, total) => `Hiển thị ${shown} trong tổng số ${total} dự án`,
        matchingCount: (count) => `${count} dự án phù hợp`,
        resetFilters: "Đặt lại bộ lọc",
        emptyTitleFilters: "Không có dự án phù hợp với bộ lọc của bạn",
        emptyDescFilters:
            "Hãy thử tìm kiếm rộng hơn hoặc xóa bộ lọc để xem tất cả dự án.",
        emptyTitleGeneral: "Các dự án đang được cập nhật",
        emptyDescGeneral:
            "Liên hệ đội ngũ của chúng tôi để trao đổi về kinh nghiệm liên quan đến dự án của bạn.",
        location: "Địa điểm",
        status: "Trạng thái",
    },
    coursesPage: {
        eyebrow: "BIM4C Academy",
        title: "Học để làm tốt hơn",
        description:
            "Các chương trình BIM thực tiễn được xây dựng từ kinh nghiệm triển khai dự án thực tế.",
        catalogueTitle: "Tìm kỹ năng tiếp theo của bạn",
        catalogueDesc:
            "So sánh các chương trình, tìm hiểu kết quả học tập và chọn trình độ phù hợp với vai trò của bạn.",
        searchLabel: "Tìm kiếm chương trình",
        searchPlaceholder: "Tìm theo kỹ năng, công cụ hoặc chương trình...",
        programmesCount: (count) =>
            `${count} chương trình`,
        durationLabel: "Thời lượng",
        levelLabel: "Trình độ",
        tuitionLabel: "Học phí",
        exploreProgramme: "Khám phá chương trình",
        emptyTitle: "Không có chương trình phù hợp với tìm kiếm của bạn",
        emptyDesc: "Hãy thử một kỹ năng khác hoặc xóa bộ lọc.",
        howWeTeachEyebrow: "Cách chúng tôi giảng dạy",
        howWeTeachTitle: "Học qua thực hành.",
        learningValues: [
            {
                title: "Thực hành theo dự án",
                text: "Bài tập, dữ liệu và mô hình được chọn lọc từ các tình huống triển khai thực tế.",
            },
            {
                title: "Cố vấn từ chuyên gia",
                text: "Phản hồi trực tiếp giúp học viên nhận diện vấn đề và cải thiện sau mỗi bài tập.",
            },
            {
                title: "Kết quả sẵn sàng cho công việc",
                text: "Sản phẩm hoàn thành có thể áp dụng ngay vào công việc hoặc dùng trong hồ sơ năng lực chuyên môn.",
            },
        ],
    },
    servicesPage: {
        eyebrow: "Năng lực BIM4C",
        title: "Giải pháp cho toàn bộ vòng đời dự án",
        description:
            "Từ chiến lược và phối hợp thiết kế đến bàn giao số, mỗi giải pháp đều được xây dựng quanh một kết quả có thể đo lường.",
        catalogueEyebrow: "Danh mục giải pháp",
        catalogueTitle: "Chuyên môn phù hợp với dự án của bạn.",
        catalogueDesc:
            "Từ chiến lược BIM đến mô hình phối hợp và thông tin tài sản, tìm hỗ trợ phù hợp với giai đoạn dự án và đội ngũ của bạn.",
        searchLabel: "Tìm kiếm giải pháp",
        searchPlaceholder: "Tìm theo tên, mục tiêu hoặc năng lực...",
        matchingCount: (count) => `${count} giải pháp phù hợp`,
        solutionBadge: "Giải pháp BIM4C",
        exploreSolution: "Khám phá giải pháp",
        emptyTitle: "Không tìm thấy giải pháp",
        emptyDesc: "Hãy thử từ khóa hoặc danh mục khác.",
        startEyebrow: "Bắt đầu với BIM4C",
        startTitle: "Biến thách thức dự án thành lộ trình triển khai rõ ràng.",
        talkToExpert: "Trao đổi với chuyên gia",
        guideEyebrow: "Tìm điểm khởi đầu của bạn",
        guideTitle: "Đội ngũ của bạn cần gì?",
        guideDesc:
            "Chọn một ưu tiên để xem điểm khởi đầu phù hợp. Chúng tôi có thể cùng nhau xác định phạm vi.",
        suggestedStartingPoint: "Điểm khởi đầu đề xuất",
        usefulForFirstConversation: "Hữu ích cho buổi trao đổi đầu tiên",
        discussNeeds: "Trao đổi nhu cầu của bạn",
        faqEyebrow: "Trước khi bắt đầu",
        faqTitle: "Một vài câu trả lời hữu ích.",
        faqDesc: "Biết trước những gì cần chuẩn bị trước khi bắt đầu trao đổi.",
        needs: [
            {
                label: "Bắt đầu với kế hoạch BIM",
                slug: "tu-van-bim",
                title: "Xác định hướng đi rõ ràng trước khi bắt đầu mô hình hóa.",
                description:
                    "Thống nhất mục tiêu dự án, yêu cầu thông tin và trách nhiệm của từng bộ phận.",
                preparation:
                    "Giai đoạn dự án, cơ cấu đội ngũ và các yêu cầu BIM hiện có (nếu có).",
            },
            {
                label: "Phối hợp mô hình dự án",
                slug: "bim-coordination",
                title: "Giúp các bộ môn làm việc trên nền thông tin đã được phối hợp.",
                description:
                    "Tổng hợp mô hình, rà soát xung đột và giúp việc theo dõi, xử lý vấn đề dễ dàng hơn.",
                preparation:
                    "Mô hình các bộ môn hiện có, mốc tiến độ dự án và các khó khăn phối hợp hiện tại.",
            },
            {
                label: "Phát triển đội ngũ của chúng tôi",
                slug: "dao-tao",
                title: "Xây dựng kỹ năng mà đội ngũ của bạn có thể áp dụng ngay vào công việc.",
                description:
                    "Trao đổi về đào tạo dựa trên kinh nghiệm hiện tại, vai trò và quy trình làm việc của đội ngũ.",
                preparation:
                    "Vai trò của đội ngũ, kinh nghiệm phần mềm và các kỹ năng bạn muốn phát triển.",
            },
            {
                label: "Chuẩn bị bàn giao số",
                slug: "digital-twin-va-du-lieu-tai-san",
                title: "Giúp thông tin dự án hữu ích ngoài giai đoạn thi công.",
                description:
                    "Xác định cách cấu trúc, kiểm tra và bàn giao dữ liệu tài sản cho mục đích sử dụng tiếp theo.",
                preparation:
                    "Yêu cầu thông tin tài sản, dữ liệu mô hình hiện có và ưu tiên vận hành.",
            },
        ],
        faqs: [
            {
                question: "Khi nào nên mời BIM4C tham gia?",
                answer:
                    "Bạn có thể bắt đầu ở giai đoạn hoạch định, thiết kế, thi công hoặc bàn giao. Hãy cho chúng tôi biết giai đoạn hiện tại và các quyết định sắp tới để cùng trao đổi về nơi hỗ trợ BIM sẽ hữu ích nhất.",
            },
            {
                question: "Cần chuẩn bị gì cho buổi trao đổi đầu tiên?",
                answer:
                    "Một bản tóm tắt ngắn về dự án, giai đoạn hiện tại, các mốc quan trọng và thách thức bạn muốn giải quyết. Mô hình hoặc yêu cầu BIM hiện có (nếu có) sẽ hữu ích, nhưng bạn không cần một bản mô tả đầy đủ để liên hệ với chúng tôi.",
            },
            {
                question: "Có thể trao đổi hỗ trợ cho một phần của dự án không?",
                answer:
                    "Có. Yêu cầu của bạn có thể tập trung vào một nhu cầu cụ thể như hoạch định BIM, phối hợp mô hình, đào tạo đội ngũ hoặc thông tin tài sản. Phạm vi và trách nhiệm sẽ được thống nhất dựa trên nhu cầu đó.",
            },
            {
                question: "Phạm vi, chi phí và sản phẩm bàn giao được thống nhất như thế nào?",
                answer:
                    "Những yếu tố này phụ thuộc vào giai đoạn dự án, quy mô, thông tin hiện có và mức độ hỗ trợ cần thiết. Buổi trao đổi ban đầu sẽ giúp làm rõ các yếu tố này trước khi lập đề xuất.",
            },
        ],
    },
    blogPage: {
        eyebrow: "Kiến thức BIM4C",
        title: "Kiến thức",
        description:
            "Những bài học thực tiễn, câu chuyện dự án và góc nhìn về tương lai của chuyển đổi số xây dựng.",
        catalogueEyebrow: "Chủ đề kiến thức",
        catalogueTitle: "Góc nhìn thực tiễn về chuyển đổi số xây dựng",
        catalogueDesc:
            "Góc nhìn chuyên gia, bài học dự án và xu hướng công nghệ cho ngành xây dựng.",
        searchLabel: "Tìm kiếm bài viết",
        searchPlaceholder: "Tìm kiếm bài viết...",
        matchingCount: (count) => `${count} bài viết phù hợp`,
        readMore: "Đọc thêm →",
        emptyTitle: "Không tìm thấy bài viết",
        emptyDesc: "Hãy thử từ khóa hoặc chủ đề khác.",
    },
    contactPage: {
        eyebrow: "Liên hệ",
        heroTitle: "Hãy cùng trao đổi về dự án của bạn.",
        heroDesc:
            "Cho chúng tôi biết bạn đang ở giai đoạn nào của dự án. Cùng nhau, chúng ta sẽ xác định hỗ trợ BIM phù hợp và các bước tiếp theo.",
        talkEyebrow: "Trao đổi với BIM4C",
        talkTitle: "Một phạm vi rõ ràng bắt đầu từ đây.",
        talkDesc:
            "Hãy cho chúng tôi biết về thách thức của bạn. Chúng tôi sẽ giúp làm rõ phạm vi, ưu tiên và các bước tiếp theo.",
        commitments: [
            "Trao đổi trực tiếp với chuyên gia BIM",
            "Đề xuất phù hợp với giai đoạn và quy mô dự án của bạn",
            "Thông tin doanh nghiệp của bạn được bảo mật",
        ],
        enquiryTitle: "Yêu cầu về dự án",
        enquiryDesc:
            "Các trường có dấu * là bắt buộc. Chúng tôi thường phản hồi trong vòng 1 ngày làm việc.",
        responseTime: "1 ngày làm việc",
        officesTitle: "Trụ sở & Trung tâm kỹ thuật BIM4C",
        danangHq: {
            title: "BIM4C tại Đà Nẵng",
            address: "20 Bắc Sơn, Thành phố Đà Nẵng",
            phone: "+84 93 2468 099",
            note: "Anh Hiếu: +84 796 879 899",
        },
        hanoiHq: {
            title: "",
            address: "",
            phone: "",
        },
        hcmcBranch: {
            title: "",
            address: "",
            phone: "",
        },
        taxInfo: {
            label: "",
            code: "",
            status: "",
            taxAddress: "",
            representative: "",
        },
        vatBilling: {
            title: "",
            desc: "",
            taxId: "",
            taxAddress: "",
            bankName: "",
            accountNumber: "",
            company: "",
            accountHolder: "",
        },
        mapSection: {
            eyebrow: "Vị trí & Chỉ đường",
            title: "Trụ sở chính BIM4C tại Đà Nẵng",
            desc: "Văn phòng BIM4C: 20 Bắc Sơn, Thành phố Đà Nẵng.",
            directionsBtn: "Chỉ đường trên Google Maps",
            copyAddressBtn: "Sao chép địa chỉ",
            workingHours: "Vui lòng liên hệ để sắp xếp lịch hẹn.",
            distanceAirport: "",
        },
    },
    detailPage: {
        onThisPage: "Trong trang này",
        enquireProgramme: "Hỏi về chương trình này",
        discussProject: "Trao đổi về dự án của bạn",
        curriculumTitle: "Chương trình khóa học",
        keepExploring: "Khám phá thêm",
        relatedContent: "Nội dung liên quan",
        viewAll: "Xem tất cả",
        byAuthor: "Bởi",
        getInsightsTitle: "Nhận thông tin BIM",
        getInsightsDesc:
            "Bài học dự án thực tiễn và kiến thức chuyển đổi số xây dựng, gửi trực tiếp đến hộp thư của bạn.",
        certificateTitle: "Chứng chỉ xác thực BIM4C",
        certificateDesc:
            "Cấp kèm mã QR xác thực và hồ sơ năng lực số sau khi hoàn thành đồ án tốt nghiệp khóa học.",
        certificateBadge: "NĂNG LỰC ĐÃ XÁC THỰC",
        projectProfile: "Hồ sơ dự án",
        courseProfile: "Tổng quan khóa học",
        serviceProfile: "Tổng quan giải pháp",
        articleInfo: "Thông tin bài viết",
        backProjects: "Tất cả dự án",
        backCourses: "Tất cả chương trình",
        backServices: "Tất cả giải pháp",
        backBlog: "Tất cả bài viết",
        softwareStack: "Phần mềm & Công nghệ sử dụng",
        b2bTrainingTitle: "Đào tạo doanh nghiệp B2B theo nhóm",
        b2bTrainingDesc:
            "Chương trình đào tạo tùy chỉnh theo mô hình dự án và quy trình làm việc thực tế của tổ chức bạn.",
        b2bTrainingAction: "Yêu cầu đào tạo doanh nghiệp",
        standardsCompliance: "Tuân thủ tiêu chuẩn quốc tế",
        deliverablesChecklist: "Danh mục sản phẩm bàn giao trên CDE",
        deliverables: [
            "Mô hình phối hợp 3D tổng hợp (ACC / BIM 360 CDE)",
            "Ma trận xung đột BCF & Báo cáo theo dõi vấn đề đa bộ môn",
            "Bản vẽ shop drawing tự động trích xuất & Dữ liệu tài sản as-built COBie",
        ],
        authorProfileTitle: "Chuyên gia biên soạn / Bộ phận nghiên cứu",
        authorRole: "Bộ phận Nghiên cứu BIM & Công nghệ số (BIM4C R&D Lab)",
        fields: {
            client: "Chủ đầu tư",
            location: "Địa điểm",
            scale: "Quy mô",
            contractPackage: "Gói thầu",
            expectedCompletion: "Dự kiến hoàn thành",
            projectYear: "Năm thực hiện",
            status: "Trạng thái",
            duration: "Thời lượng",
            level: "Trình độ",
            price: "Học phí",
            instructor: "Giảng viên",
        },
        trustSignals: {
            ndaTitle: "Cam kết bảo mật thông tin (NDA)",
            ndaDesc:
                "Chúng tôi ký thỏa thuận bảo mật trước khi rà soát dữ liệu mô hình và bản vẽ mang tính bảo mật.",
            slaTitle: "Tư vấn trực tiếp",
            slaDesc: "Liên hệ BIM4C để thống nhất phạm vi và lịch tư vấn.",
            expertTitle: "Đội ngũ BIM Manager cấp cao",
            expertDesc: "Trao đổi trực tiếp với đội ngũ tư vấn và triển khai của BIM4C.",
        },
    },
    forms: {
        fullName: "Họ và tên *",
        fullNamePlaceholder: "Họ và tên của bạn",
        workEmail: "Email *",
        workEmailPlaceholder: "ban@congty.com",
        phone: "Số điện thoại (không bắt buộc)",
        phonePlaceholder: "090 000 0000",
        company: "Công ty (không bắt buộc)",
        companyPlaceholder: "Tổ chức của bạn",
        message: "Chúng tôi có thể hỗ trợ gì? *",
        messagePlaceholder:
            "Bạn đang thực hiện dự án gì, và bạn muốn chúng tôi hỗ trợ ở đâu?",
        consentLabel: "Tôi đã đọc và đồng ý với",
        privacyPolicy: "Chính sách bảo mật",
        consentSuffix: "và việc xử lý dữ liệu cá nhân của tôi.",
        submitEnquiry: "Gửi yêu cầu dự án",
        submitRegistration: "Yêu cầu thông tin chương trình",
        submitting: "Đang gửi...",
        registrationNotice:
            "Chúng tôi sẽ liên hệ với bạn về lịch khai giảng, học phí và các bước tiếp theo. Việc gửi biểu mẫu này không đồng nghĩa với việc giữ chỗ.",
        thankYouTitle: "Cảm ơn bạn đã liên hệ.",
        thankYouDesc:
            "Đội ngũ của chúng tôi sẽ xem xét yêu cầu và liên hệ với bạn qua thông tin bạn đã cung cấp.",
        sendAnother: "Gửi yêu cầu khác",
        dataProtectionNote:
            "BIM4C chỉ sử dụng thông tin này để phản hồi yêu cầu của bạn.",
    },
    legalPage: {
        eyebrow: "THÔNG TIN PHÁP LÝ",
        heroTitle: "Minh bạch trong từng cam kết",
        heroDesc:
            "Chính sách và điều khoản áp dụng khi bạn truy cập, tương tác và chia sẻ thông tin với BIM4C.",
        sectionEyebrow: "PHÁP LÝ BIM4C",
        sectionTitle: "Thông tin bạn nên biết",
        sectionDesc:
            "Chọn một tài liệu bên dưới để tìm hiểu cách chúng tôi vận hành website này và bảo vệ quyền lợi của bạn với tư cách người dùng.",
        readDocument: "Đọc tài liệu",
    },
    bimViewerPage: {
        badge: "Trình xem IFC · Three.js & Web-IFC",
        title: "Trình xem mô hình BIM 3D trực tuyến tương tác",
        subtitle:
            "Tải mô hình IFC, kiểm tra thuộc tính gốc, đo đạc bề mặt hiển thị và khám phá mặt cắt.",
        selectModel: "Chọn mô hình",
        uploadIfc: "Tải lên tệp .IFC của bạn",
        uploadIfcNotice:
            "Tệp IFC được xử lý cục bộ ngay trên trình duyệt của bạn và không được tải lên máy chủ.",
        tools: {
            orbit: "Xoay & Chọn",
            measure: "Thước đo 3D",
            section: "Mặt cắt",
            explode: "Chế độ tách rời",
            layers: "Lớp bộ môn",
            clashes: "Xung đột",
            reset: "Vừa khung nhìn",
            fullscreen: "Toàn màn hình",
            snapshot: "Lưu ảnh 3D",
        },
        views: {
            perspective: "Phối cảnh 3D",
            top: "Mặt bằng",
            front: "Mặt đứng",
            right: "Mặt bên",
            isometric: "Hình chiếu trục đo (ISO)",
        },
        layers: {
            architecture: "Kiến trúc (Arch)",
            structure: "Kết cấu (Struct)",
            mep: "Cơ điện công trình (MEP)",
            clashMarkers: "Điểm đánh dấu xung đột",
        },
        sections: {
            title: "Cắt mặt phẳng theo trục",
            axisX: "Mặt cắt theo trục X",
            axisY: "Mặt cắt theo trục Y",
            axisZ: "Mặt cắt theo trục Z",
            resetClipping: "Đặt lại mặt cắt",
        },
        measure: {
            title: "Đo khoảng cách 3D",
            instruction:
                "Nhấp chọn 2 điểm trên bề mặt mô hình 3D để đo khoảng cách thực tế.",
            distance: "Khoảng cách Euclid thực (L)",
            deltaX: "Độ lệch trục X (ΔX)",
            deltaY: "Độ lệch trục Y (ΔY)",
            deltaZ: "Độ lệch trục Z (ΔZ)",
            clear: "Xóa phép đo",
        },
        explode: {
            title: "Chế độ xem tách rời mô hình",
            intensity: "Mức độ / Cường độ tách rời",
        },
        properties: {
            title: "Trình kiểm tra thuộc tính IFC",
            noSelection: "Chưa chọn đối tượng",
            noSelectionDesc:
                "Nhấp vào bất kỳ cấu kiện 3D nào để kiểm tra thuộc tính IFC và các Pset kỹ thuật.",
            elementName: "Tên cấu kiện",
            ifcType: "Loại IFC",
            guid: "Mã định danh toàn cục (GUID)",
            storey: "Tầng công trình",
            psetsTitle: "Tập hợp thuộc tính (Psets)",
            geometryTitle: "Hình học và thuộc tính gốc",
            spatialTree: "Cây phân cấp không gian",
        },
        clashes: {
            title: "Ví dụ các vấn đề phối hợp",
            count: (count) => `${count} vấn đề phối hợp`,
            severity: {
                high: "Nghiêm trọng (Cao)",
                medium: "Trung bình",
                low: "Khuyến nghị (Thấp)",
            },
            focusClash: "Phóng đến vị trí xung đột",
        },
        performance: {
            fps: "FPS",
            elements: "Cấu kiện",
            memory: "Bộ đệm hình học",
            openBimStandard: "Trình xem mô hình IFC",
        },
    },
};

export const viDictionary = repairMojibake(viDictionaryRaw) as Dictionary;
